...
GRID = [
    ...
    [2, 49, 49, 70, 31, 47, 98, 26, 55, 36, 17,
        39, 56, 80, 52, 36, 42, 69, 73, 70],
    ...
    [0, 18, 14, 11, 63, 45, 67, 12, 99, 76, 31,
        31, 89, 47, 99, 20, 39, 23, 90, 54],
    [40, 57, 29, 42, 89, 2, 10, 20, 26, 44, 67, 47, 7, 69, 16, 72, 11, 88, 1, 69],
    [0, 60, 93, 69, 41, 44, 26, 95, 97, 20, 15, 55, 5, 28, 7, 3, 24, 34, 74, 16],
    [75, 87, 71, 24, 92, 75, 38, 63, 17, 45,
        94, 58, 44, 73, 97, 46, 94, 62, 31, 92],
    [4, 17, 40, 68, 36, 33, 40, 94, 78, 35, 3, 88, 44, 92, 57, 33, 72, 99, 49, 33],
    [5, 40, 67, 56, 54, 53, 67, 39, 78, 14, 80,
        24, 37, 13, 32, 67, 18, 69, 71, 48],
    [7, 98, 53, 1, 22, 78, 59, 63, 96, 0, 4, 0, 44, 86, 16, 46, 8, 82, 48, 61],
    [78, 43, 88, 32, 40, 36, 54, 8, 83, 61, 62,
        17, 60, 52, 26, 55, 46, 67, 86, 43],
    [52, 69, 30, 56, 40, 84, 70, 40, 14, 33,
        16, 54, 21, 17, 26, 12, 29, 59, 81, 52],
    ...
    [91, 62, 36, 36, 13, 12, 64, 94, 63, 33,
        56, 85, 17, 55, 98, 53, 76, 36, 5, 67],
    [8, 0, 65, 91, 80, 50, 70, 21, 72, 95, 92, 57, 58, 40, 66, 69, 36, 16, 54, 48]
]


def up(position: tuple, depth: int) -> List[int]:
    ...
    """
    return [GRID[position[0]][position[1] - i] for i in range(0, depth)]


def diagonal_ru(position: tuple, depth: int) -> List[int]:
    ...
    """
    return [GRID[position[0] + i][position[1] - i] for i in range(0, depth)]


def right(position: tuple, depth: int) -> List[int]:
    ...
    """
    return [GRID[position[0] + i][position[1]] for i in range(0, depth)]


def diagonal_rd(position: tuple, depth: int) -> List[int]:
...
def down(position: tuple, depth: int) -> List[int]:
...
def diagonal_ld(position: tuple, depth: int) -> List[int]:
...
def left(position: tuple, depth: int) -> List[int]:
...
def diagonal_lu(position: tuple, depth: int) -> List[int]:
